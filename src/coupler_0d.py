...
def append_progress(paths: Paths, it: int, stats: dict, tol: float):
    ...
    """
    import csv
    src_path = paths.coupled / "convergence_inlet.csv"
    write_header = not src_path.exists()
    with open(src_path, "a", newline="") as f:
        w = csv.writer(f)
        if write_header:
    ...
    snk_path = paths.coupled / "convergence_outlet.csv"
    write_header = not snk_path.exists()
    with open(snk_path, "a", newline="") as f:
        w = csv.writer(f)
        if write_header:
            w.writerow(["iter", "max", "mean", "median", "p95"])
    ...
    if plt is not None:
        try:
            ...
            plt.grid(True)
            fig_path = paths.coupled / "convergence_inlet.png"
    ...
    if plt is not None:
        try:
            import pandas as pd
            df = pd.read_csv(snk_path)
            plt.figure()
            plt.plot(df["iter"], df["max"], marker="o")
            plt.axhline(tol, linestyle="--")
            plt.xlabel("Iteration (k)")
            plt.ylabel(r"max (|q_k - q_{k+1}| / |q_k|), ε)")
            plt.title("Coupling convergence (outlet)")
            plt.grid(True)
            fig_path = paths.coupled / "convergence_outlet.png"
            plt.savefig(fig_path, dpi=150, bbox_inches="tight")
            plt.close()
        except Exception:
            pass
...
def main():
    ...
    for k in range(args.max_iters):
        ...
        if k%2 == 0:
            print("[info] Even iteration: updating the venous branch.")
            ...
            shutil.copytree(paths.coupled / f"run_{k}/inlet/",
                            paths.coupled / f"run_{k+1}/inlet/")
            shutil.copy2(paths.coupled / f"run_{k}/outlet/plot_0d_results_to_3d.py",
                         paths.coupled / f"run_{k+1}/outlet/plot_0d_results_to_3d.py")
            shutil.copy2(paths.coupled / f"run_{k}/outlet/geom.csv",
                         paths.coupled / f"run_{k+1}/outlet/geom.csv")
            subprocess.run("python plot_0d_results_to_3d.py", cwd=paths.coupled / f"run_{k+1}/outlet/", shell=True)          

        else:
            print("[info] Odd iteration: Updating the arterial branch.")
            assign_path = Path(args.assign_pressures_script)
            update_0d.outlet_stage_iter(
                update_0d.Paths(paths.root, paths.geometry, paths.voronoi, paths.solves, paths.coupled),
                run_i=k, run_next=k+1,
            ...
            shutil.copytree(paths.coupled / f"run_{k}/outlet/",
                         paths.coupled / f"run_{k+1}/outlet/")
            shutil.copy2(paths.coupled / f"run_{k}/inlet/plot_0d_results_to_3d.py",
                paths.coupled / f"run_{k+1}/inlet/plot_0d_results_to_3d.py")
            shutil.copy2(paths.coupled / f"run_{k}/inlet/geom.csv",
                paths.coupled / f"run_{k+1}/inlet/geom.csv")
            subprocess.run("python plot_0d_results_to_3d.py", cwd=paths.coupled / f"run_{k+1}/inlet/", shell=True)
